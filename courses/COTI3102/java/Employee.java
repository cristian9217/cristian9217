...
public class Employee 
{
	...
	public void setName(String name) 
	{
		if(!Character.isUpperCase(name.charAt(0)))
			die("First name starts with uppercase: " + lastName.charAt(0));
		
		if(name.length() < 2 || name.length() > 9)
			die("First name must be between 2 and 9 characters long.");
	...
	public void setLastName(String lastName) 
	{
		...
		if(lastName.length() < 2 || lastName.length() > 9)
			die("Last name must be between 2 and 9 characters long.");
	...
	*/
	...
	{
		// Constants defining the valid hours range.
	...
	{
	    // Constants defining the valid pay rate range
		final double MIN_PAY_RATE = 7.00;
		final double MAX_PAY_RATE = 50.00;
		
		if(payRate < MIN_PAY_RATE || MAX_PAY_RATE > 50.00)
